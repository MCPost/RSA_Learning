...
ROI = {'OCC','TMP'}; r = 1;
MSize = {'Dat128','Dat16'}; sz = 2;
Cat = {'Perceptual','Semantic'}; c = 1;
SubCat = [1 2; 3 4];
Measure = {'Meas128','Meas16'}; m = 3;

for d = 1:length(datanames)
    for r = 1:length(ROI)
        for sz = 2:length(MSize)
            for c = 1:length(Cat)
                for sc = 1:size(SubCat,1)
                    for m = 1:5
                        figure('Pos', [325 510 650 402]);
                        plot([]); hold on
                        Dat_names = fieldnames(RSA_DiffParams.(datanames{d}).(ROI{r}).(MSize{sz}).(Cat{c}));
                        dat1 = squeeze(RSA_DiffParams.(datanames{d}).(ROI{r}).(MSize{sz}).(Cat{c}).(Dat_names{SubCat(sc,1)})(m,:,:));
                        dat2 = squeeze(RSA_DiffParams.(datanames{d}).(ROI{r}).(MSize{sz}).(Cat{c}).(Dat_names{SubCat(sc,2)})(m,:,:));
                        ...
                        hold off
                        if(isempty(regexp(datanames{d},'_MNN','ONCE')))
                            title_str = strrep(datanames{d},'_NoMNN','_MNN');
                        else
                            title_str = strrep(datanames{d},'_MNN','_NoMNN');
                        end
                        ylabel(RSA_DiffParams.(Measure{sz}){m}); xlabel('Time (s)'); title(strrep(title_str,'_',' '))
                        xlim([-0.2 1.5]);lg = legend([h1 h2], {[Cat{c},' ',Dat_names{SubCat(sc,1)}],[Cat{c},' ',Dat_names{SubCat(sc,2)}]}); legend boxoff; set(lg,'FontSize',14)
                        box off; %ylim([0.35 0.6])
                        set(gca,'linewidth',2.5,'FontSize',14)
                        saveas(gcf,sprintf('Results/DiffParams/%s_%s_%s_%s_%s_%s.png',ROI{r},MSize{sz},Cat{c},Dat_names{SubCat(sc,1)},RSA_DiffParams.(Measure{sz}){m},title_str))
                        close(gcf)
                    end
                end
            end
...
for d = 1:12 %16:length(datanames)
    for sz = 2%:length(MSize)
        for m = 1:4
            figure('Pos', [325 510 650 402]);
            plot([]); hold on
            Dat_names = fieldnames(RSA_DiffParams.(datanames{d}).(ROI{r}).(MSize{sz}).(Cat{c}));
            dat1 = squeeze(RSA_DiffParams.(datanames{d}).(ROI{r1}).(MSize{sz}).(Cat{c1}).(Dat_names{SubCat(2,2)})(m,:,:)) - squeeze(RSA_DiffParams.(datanames{d}).(ROI{r1}).(MSize{sz}).(Cat{c1}).(Dat_names{SubCat(2,1)})(m,:,:));
            dat2 = squeeze(RSA_DiffParams.(datanames{d}).(ROI{r2}).(MSize{sz}).(Cat{c2}).(Dat_names{SubCat(2,2)})(m,:,:)) - squeeze(RSA_DiffParams.(datanames{d}).(ROI{r2}).(MSize{sz}).(Cat{c2}).(Dat_names{SubCat(2,1)})(m,:,:));
...
%                 end
            ...
            ylabel(['Diff ',RSA_DiffParams.(Measure{sz}){m}]); xlabel('Time (s)'); title(strrep(title_str,'_',' '))
            lg = legend([h1 h2], {[ROI{r1},' ',Cat{c1},' BT - WI'],[ROI{r2},' ',Cat{c2},' BT - WI']}); legend boxoff; set(lg,'FontSize',14)
            box off;
            set(gca,'linewidth',2.5,'FontSize',14)
            saveas(gcf,sprintf('Results/DiffParams/%s_%s_%s_%s_%s_%s_%s.png',ROI{r1},ROI{r2},MSize{sz},Cat{c1},Cat{c2},RSA_DiffParams.(Measure{sz}){m},title_str))
            close(gcf)
        end
    end
end
