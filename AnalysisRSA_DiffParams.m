            ...
            for c = 1:length(Cat)
                for sc = 1:size(SubCat,1)
                    for m = 1:5
                        ...
                        saveas(gcf,sprintf('Results/DiffParams/%s_%s_%s_%s_%s_%s.png',ROI{r},MSize{sz},Cat{c},Dat_names{SubCat(sc,1)},RSA_DiffParams.(Measure{sz}){m},datanames{d}))
                        close(gcf)
                    end
                end
            end
...
%             end
            ...
            h2 = plot(RSA_DiffParams.TimeVec, nanmean(dat2,1),'r','linewidth',2);
            hold off
            ylabel(['Diff ',RSA_DiffParams.(Measure{sz}){m}]); xlabel('Time (s)'); %title([Cat{c},': ',Dat_names{dt(1)},' vs ',Dat_names{dt(2)}])
            ...
            %close(gcf)
            saveas(gcf,'Results/DiffParams/TMP_PerceptualvsSemantic_BT-WI_Dat16_eucl.png')
            close(gcf)
        end
    end
end
